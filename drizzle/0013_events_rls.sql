...
-- Row-Level Security para `events` (issue #18).
-- Mesmo padrão das migrations 0001, 0004, 0006, 0008 e 0010: ENABLE + FORCE + policy de
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'app_rls') THEN
    GRANT SELECT, INSERT, UPDATE, DELETE ON "events" TO app_rls;
  ELSE
