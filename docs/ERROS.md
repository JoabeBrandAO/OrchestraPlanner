...
### 2026-08-15 — Subconsulta correlacionada escrita à mão devolvia zero em silêncio
- **Contexto:** `listAccounts` (#52) calculava o saldo com uma subconsulta correlacionada dentro do template `sql` do Drizzle: `coalesce((select sum(...) from transactions where transactions.account_id = accounts.id), 0)`.
- **O que ocorreu:** o saldo vinha sempre igual ao saldo inicial. Um `select sum(...)` disparado direto na mesma sessão devolvia 700; a mesma soma dentro do `select` da consulta principal devolvia 0.
- **Causa raiz:** a correlação não se estabeleceu — a referência à tabela externa dentro do template não amarrou no `from` principal. O sintoma é o pior possível: **nenhum erro**, apenas zero, que é um número perfeitamente plausível para um saldo.
- **Correção:** trocar por agregação idiomática do query builder — `left join` nos lançamentos + `group by accounts.id` + `sum(case when ...)`. Continua uma consulta só, e conta sem lançamento sobrevive ao `left join` somando zero de verdade.
- **Como evitar a recorrência:** (1) preferir o construtor de consultas do Drizzle a SQL escrito à mão no template — o construtor conhece os aliases, o template não; (2) quando o valor for derivado, o teste tem que exigir um número **específico** (`toBe(31950)`), nunca "existe" ou "é número" — zero passa em asserção frouxa; (3) o teste de integração pegou isto antes do PR justamente porque afirmava o valor exato.

### 2026-09-04 — `sslmode=require` não verifica nada, e `channel_binding` derruba a conexão
- **Contexto:** auditoria de segurança (#71). A conexão com o Neon usava `sslmode=require` desde a Phase B, com a leitura implícita de que "require" seria o modo estrito.
- **O que ocorreu:** nada quebrou — e é esse o problema. `require` **criptografa mas não valida o certificado**: protege contra escuta passiva e não contra quem se apresente como o banco. No driver `postgres.js` isso é explícito no código (`connection.js`): para `require`, `allow` e `prefer` ele define `rejectUnauthorized: false`; qualquer **outro** valor cai no padrão do Node, que verifica cadeia e nome do host.
- **Causa raiz:** o nome do modo engana. "Require" responde a "exigir TLS?", não a "verificar quem está do outro lado?" — e a documentação de banco gerenciado costuma dar a string com `require` pronta para copiar.
- **Correção:** `sslmode=verify-full` em `.env`, `.env.example` e nos segredos do CI. A suíte de RLS contra o Neon passou sem nenhuma outra mudança.
- **Armadilha adjacente:** **não** acrescentar `channel_binding=require` na string da aplicação. O `postgres.js` repassa todo parâmetro de query que ele não conhece dentro do **pacote de conexão**, e o servidor recusa com `unrecognized configuration parameter`. Aquele parâmetro é da string de `psql`/libpq, não da do driver.
- **Como evitar a recorrência:** (1) tratar `sslmode` como decisão de segurança, não como detalhe de string de conexão — o valor seguro é `verify-full`; (2) antes de copiar parâmetro de conexão da documentação do provedor, conferir se o **driver** o entende: no `postgres.js`, só `sslmode` e as opções listadas em `defaults` são interpretadas, o resto vira parâmetro de startup.

### 2026-09-04 — Proteção de branch que não protege quem administra
- **Contexto:** achado S6 da auditoria (#70). A `main` estava sem proteção nenhuma num repositório público com merge autônomo; liguei branch protection exigindo PR e o check do CI, com `enforce_admins: false` para o dono manter uma saída de emergência.
- **O que ocorreu:** o teste do critério de aceite — tentar um push direto na `main` — **passou**. O commit (vazio) entrou. Repeti depois de corrigir, e aí veio o esperado: `GH006: Protected branch update failed... Changes must be made through a pull request`.
- **Causa raiz:** `enforce_admins: false` isenta das regras exatamente quem administra o repositório — e, num projeto de uma pessoa só, quem administra é quem empurra. A proteção existia para todo mundo, menos para as duas únicas mãos que tocam o repositório (o dono e o agente usando o token dele).
- **Correção:** `enforce_admins: true` (endpoint `POST .../protection/enforce_admins`; o `PUT` responde 404). A saída de emergência continua existindo — desligar a proteção é um clique — mas passa a ser um ato deliberado em vez de um privilégio silencioso.
